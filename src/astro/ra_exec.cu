...
int cura_fw(CURAFFT_PLAN *plan, ragridder_plan *gridder_plan)
{
    ...
    if (gridder_plan->kv.weight != NULL)
    {
        ...
        checkCudaError(cudaMalloc((void **)&d_wgt, sizeof(PCS) * nrow));
        checkCudaError(cudaMemcpy(d_wgt, gridder_plan->kv.weight, sizeof(PCS) * nrow, cudaMemcpyHostToDevice));

        matrix_elementwise_divide_invoker(plan->d_c, d_wgt, nrow);
        checkCudaError(cudaFree(d_wgt)); // to save memory
    }
    int ier = 0;
    int flag = plan->iflag;
...
int cura_prestage(CURAFFT_PLAN *plan, ragridder_plan *gridder_plan)
{
    ...
    if (plan->iflag == 1)
    {
        ...
        checkCudaError(cudaMalloc((void **)&plan->d_x, sizeof(PCS) * (N1 / 2 + 1) * (N2 / 2 + 1)));
    ...
    if (plan->mem_limit)
    {
        if (plan->iflag == 1)
        {
            ...
            if (plan->opts.gpu_gridder_method == 6)
                nzp = (int)ceil(plan->copts.kw / 2.0);
            ...
            checkCudaError(cudaMalloc((void **)&plan->fw_temp, plan->nf1 * plan->nf2 * sizeof(CUCPX))); /// free somewhere | for dft
            checkCudaError(cudaMemset(plan->fw_temp, 0, plan->nf1 * plan->nf2 * sizeof(CUCPX)));
        }
        else
        {
    ...
    else if (plan->opts.gpu_gridder_method != 0)
        bin_mapping(plan, gridder_plan->d_uvw); //currently just support 3d //uvw or u?
    ...
    */
    ...
    if (plan->execute_flow == 1)
    {
        ...
        if (plan->mem_limit)
        {
            ...
            if (plan->opts.gpu_gridder_method == 6)
            {
                checkCudaError(cudaMemcpy(plan->hive_count + nhive[0] * nhive[1] * nhive[2] * 2 + 2, gridder_plan->temp_station, sizeof(int) * (nhive[0] * nhive[1] * (int)ceil(plan->copts.kw / 2.0) + 1), cudaMemcpyHostToDevice));
                nzu = (int)ceil(plan->copts.kw / 2.0);
            }
            else
                checkCudaError(cudaMemcpy(plan->hive_count + nhive[0] * nhive[1] * nhive[2] * 2 + 2, gridder_plan->temp_station, sizeof(int) * (nhive[0] * nhive[1] + 1), cudaMemcpyHostToDevice));
            ...
            checkCudaError(cudaFree(plan->d_u_out));
            checkCudaError(cudaFree(plan->d_v_out));
            checkCudaError(cudaFree(plan->d_w_out));
            checkCudaError(cudaFree(plan->d_c_out));
            plan->fw = plan->fw_temp;
        }
    ...
    else if (plan->execute_flow == 2)
    {
        ...
        for (int i = 0; i < gridder_plan->num_w; i += plan->batchsize)
        {
            ...
            checkCudaError(cudaMemset(plan->fw, 0, plan->batchsize * plan->nf1 * plan->nf2 * sizeof(CUCPX)));
            // 1. convlution
            curafft_conv(plan);
        }
    }
...
int exec_dirty2vis(CURAFFT_PLAN *plan, ragridder_plan *gridder_plan)
{
    ...
    else
    {
        ...
        for (i = 0; i < (plan->mem_limit - 1) / plan->nf3; i++)
        {
            ...
            checkCudaError(cudaMemcpy(gridder_plan->temp_station + 1, plan->histo_count + nhive[0] * plan->hivesize[0] * nhive[1] * plan->hivesize[1] * (int)ceil(plan->copts.kw / 2), sizeof(int), cudaMemcpyDeviceToHost));
            ...
            checkCudaError(cudaMemcpy(gridder_plan->temp_station + 2, plan->histo_count + histo_count_size - nhive[0] * plan->hivesize[0] * nhive[1] * plan->hivesize[1] * (int)ceil(plan->copts.kw / 2), sizeof(int), cudaMemcpyDeviceToHost));
        ...
        if (plan->opts.gpu_kerevalmeth)
            checkCudaError(cudaFree(plan->c0));
        // checkCudaError(cudaFree(plan->hive_count));
        checkCudaError(cudaFree(plan->idxnupts));
        plan->fw = plan->fw_temp;
