...
int fourier_series_appro_invoker(PCS *fseries, conv_opts opts, int N, int func_type)
{
    ...
    */
    ...
    {                                  // set up nodes z_n and vals f_n
        ...
        PCS phi = 0.0;
        if (abs(x[n]) <= opts.ES_halfwidth)
            phi = exp(opts.ES_beta * (sqrt(1.0 - opts.ES_c * x[n] * x[n]) - func_type));
        g[n] = alpha * (PCS)w[n] * phi; // vals & quadr wei
        // a[n] = exp(2 * PI * IMA * (PCS)(nf / 2 - z[n]) / (PCS)nf); // phase winding rates
    }
    double *d_x;
    PCS *d_g;
    checkCudaError(cudaMalloc((void **)&d_x, sizeof(double) * p)); // change to constant memory
    checkCudaError(cudaMalloc((void **)&d_g, sizeof(PCS) * p));

    checkCudaError(cudaMemcpy(d_x, x, sizeof(double) * p, cudaMemcpyHostToDevice));
    checkCudaError(cudaMemcpy(d_g, g, sizeof(PCS) * p, cudaMemcpyHostToDevice));

    int blocksize = 512;
    fourier_series_appro<<<(N - 1) / blocksize + 1, blocksize>>>(fseries, N, d_g, d_x, p);
    checkCudaError(cudaDeviceSynchronize());
    checkCudaError(cudaFree(d_g));
...
int fourier_series_appro_invoker(PCS *fseries, PCS *k, conv_opts opts, int N, int nf, int func_type)
{
    ...
    */
    ...
    double *d_x;
    PCS *d_g;
    checkCudaError(cudaMalloc((void **)&d_x, sizeof(double) * 2 * p)); // change to constant memory
    checkCudaError(cudaMalloc((void **)&d_g, sizeof(PCS) * 2 * p));

    checkCudaError(cudaMemcpy(d_x, x, sizeof(double) * 2 * p, cudaMemcpyHostToDevice));
    checkCudaError(cudaMemcpy(d_g, g, sizeof(PCS) * 2 * p, cudaMemcpyHostToDevice));
